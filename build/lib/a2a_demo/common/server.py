...
class TaskManager:
    """Base class for task managers that process agent tasks."""
    ...
    ) -> AsyncIterable[JSONRPCResponse] | JSONRPCResponse:
        """Handle a send task with streaming request."""
        raise NotImplementedError()
    
    async def on_set_task_push_notification(
        self, request: SetTaskPushNotificationRequest
    ) -> JSONRPCResponse:
        """Handle a set push notification request."""
        raise NotImplementedError()
    
    async def on_get_task_push_notification(
        self, request: GetTaskPushNotificationRequest
    ) -> JSONRPCResponse:
        """Handle a get push notification request."""
        raise NotImplementedError()
    
    async def on_resubscribe_to_task(
        self, request: TaskResubscriptionRequest
    ) -> AsyncIterable[JSONRPCResponse] | JSONRPCResponse:
        """Handle a task resubscription request."""
        raise NotImplementedError()


class A2AServer:
    """Implementation of an A2A protocol server."""
    ...
    def _handle_exception(self, e: Exception) -> JSONResponse:
        """Handle exceptions when processing requests."""
    ...
    def _create_response(self, result: Any) -> JSONResponse | EventSourceResponse:
        """Create an appropriate response from the task manager result."""
